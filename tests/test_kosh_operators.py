...
class TestUQMethods(unittest.TestCase):
    def setUp(self):
        ...
        seed = 20200221
        surrogate_model = {
            name: GaussianProcessRegressor().fit(X, Y[:, i])
        ...
        self.dataset = store.create("uq_data")
    ...
    def test_kosh_default_mcmc(self):
    	prior_chain_actual, my_mcmc_obj = KoshMCMC(self.dataset["inputs"],
                                                   method="default_mcmc",
                                                   input_names=input_names,
    					                           inputs_low=[0.0]*Ndim,
                                                   inputs_high=[1.0]*Ndim,
                                                   proposal_sigma=[0.2]*Ndim,
    					                           prior=[sts.beta(2, 2).pdf]*Ndim,
                                                   outputs=self.dataset["outputs"],
                                                   output_names=output_names,
                                                   quantity='x',
                                                   surrogate_model=dataset["surrogate"],
                                                   observed_value=[.5]*2,
                                                   observed_std=[.1]*2,
                                                   inputs=input_names,
                                                   total=10,
                                                   ...
                                                   seed=15,
                                                   flattened=False)[:]
        prior_chain_expected = {'input_A': np.array([[0.32151482, 0.44159015, 0.28441874, 0.42174969, 0.3671779 ,
        ...
        chains, my_mcmc_obj = KoshMCMC(self.dataset["inputs"],
                                       method="default_mcmc",
                                       input_names=input_names,
                                       inputs_low=[0.0]*Ndim,
                                       inputs_high=[1.0]*Ndim,
        ...
        posterior_chains_actual = {'input_A': array([0.16435425, 0.16435425, 0.16435425, 0.16435425, 0.11053923,
                                                     0.11053923, 0.26248796, 0.42028082, 0.51631524, 0.51631524]),
                                   'input_B': array([0.15977588, 0.15977588, 0.15977588, 0.15977588, 0.37342699,
                                                     0.37342699, 0.18809968, 0.21884804, 0.13397646, 0.13397646]),
                                   'input_C': array([0.29237039, 0.29237039, 0.29237039, 0.29237039, 0.21112243,
                                                     0.21112243, 0.11679129, 0.04605797, 0.04880956, 0.04880956])}
        for name in input_names:
            np.testing.assert_array_almost_equal(prior_chain_actual[name],
                                                 prior_chain_expected[name])
            np.testing.assert_array_almost_equal(post_chain_actual[name],
                                                 post_chain_expected[name])


    def test_kosh_discrepancy_mcmc(self):
        start = {name: .5 for name in input_names}
        start['tau_x'] = .5
        prior_chains_expected, my_mcmc_obj = KoshMCMC(self.dataset["inputs"],
                                                      ...
                                                      seed=15,
                                                      flattened=False)
        prior_chains_actual = {'input_A': np.array([[0.46345716, 0.50974877, 0.63393836, 0.63393836, 0.71894928,
        ...
        posterior_chains_expected, my_mcmc_obj = KoshMCMC(self.dataset["inputs"],
                                                          method="discrepancy_mcmc",
                                                          input_names=input_names,
                                                          inputs_low=[0.0]*Ndim,
                                                          inputs_high=[1.0]*Ndim,
                                                          ...
                                                          seed=15,
                                                          flattened=True)
        posterior_chains_actual = {'input_A': array([0.17669621, 0.17669621, 0.3008858 , 0.40131393, 0.40131393,
                                                     0.34298819, 0.67718792, 0.63688562, 0.63688562, 0.51763303]),
                                   'input_B': array([0.23788465, 0.23788465, 0.22586234, 0.75287632, 0.75287632,
                                                     0.8821385 , 0.54468051, 0.52673767, 0.52673767, 0.33972996]),
                                   'input_C': array([0.77980347, 0.77980347, 0.60195062, 0.15267065, 0.15267065,
                                                     0.44959846, 0.41060721, 0.38567044, 0.38567044, 0.43959737]),
                                   'tau_x': array([0.3891361 , 0.3891361 , 0.41292263, 0.44855446, 0.44855446,
                                                   0.53568224, 0.46209016, 0.46018472, 0.46018472, 0.47521032])}
        for name in input_names:
            np.testing.assert_array_almost_equal(prior_chains_actual[name],
                                                 prior_chains_expected[name])
            np.testing.assert_array_almost_equal(posterior_chains_actual[name],
                                                 posterior_chains_expected[name])
